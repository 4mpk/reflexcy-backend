...
namespace MohaProject.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("MohaProject") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "MohaProject", DisplayName = "MohaProject API", Resources = { "MohaProject" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["MohaProject_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["MohaProject_Web:RootUrl"]!.EnsureEndsWith('/');

            /* MohaProject_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["MohaProject_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["MohaProject_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["MohaProject_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        //Mobile Client
        var mobileClientId = configurationSection["MohaProject_WebPublic:ClientId"];
        if (!mobileClientId.IsNullOrWhiteSpace())
        {
            var mobileClientRootUrl = configurationSection["MohaProject_WebPublic:RootUrl"].Replace("_", "-");
            await CreateApplicationAsync(
                name: mobileClientId,
                type: OpenIddictConstants.ClientTypes.Public,
                consentType: OpenIddictConstants.ConsentTypes.Implicit,
